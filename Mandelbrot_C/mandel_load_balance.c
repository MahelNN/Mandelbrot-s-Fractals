...
static void error_options () {
  ...
  fprintf (stderr, "-f \t\t File \t\t Image/mandel.ppm\n");
  exit (1);
}
...
static void Compute (Image * im, int nb_iter, double x_min, double x_max, double y_min, double y_max) {
  ...
  time_file = fopen("Data/time_per_line.dat", "a");
  
  for (l = 0; l < im -> nb_rows; l ++) {

    clock_gettime ( CLOCK_MONOTONIC , & tstart ) ; /* timer */
    ...
    elap_time =( tend.tv_sec-tstart.tv_sec) +(tend.tv_nsec-tstart.tv_nsec)/1e9f;
    fprintf(time_file, "%d %2.9lf \n", l, elap_time);
