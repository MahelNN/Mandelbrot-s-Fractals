...
static void error_options()
{
  ...
  fprintf(stderr, "-f \t\t File \t\t Image/mandel_openmp_master_worker.ppm\n");
  exit(1);
}
...
static void analyzis(int argc, char **argv, int *nb_iter, double *x_min, double *x_max, double *y_min, double *y_max, int *width, int *height, char **path)
{
  ...
  *width = 1024;
  *height = 768;
  *path = "Image/mandel_openmp_master_worker.ppm";
...
int main(int argc, char **argv)
{
  ...
  Tserial = Chose_Tserial(nb_iter);
  S = Tserial / Tparallel;
  E = S / omp_get_max_threads();

  fptr = fopen("Data/01speedup_master_worker.dat", "a"); //"a" if the file doesnt exit creat it
