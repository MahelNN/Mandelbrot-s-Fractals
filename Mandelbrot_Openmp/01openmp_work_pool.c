...
static void error_options () {
  ...
  fprintf (stderr, "-f \t\t File \t\t Image/mandel_openmp_work_pool.ppm\n");
  exit (1);
}
...
static void analyzis (int argc, char * * argv, int * nb_iter, double * x_min, double * x_max, double * y_min, double * y_max, int * width, int * height, char * * path) {
  ...
  * width = 1024;
  * height = 768;
  * path = "Image/mandel_openmp_work_pool.ppm";
...
int main (int argc, char * * argv) {
  ...
  fptr = fopen("Data/01speedup_work_pool.dat", "a"); //"a" if the file doesnt exit creat it
